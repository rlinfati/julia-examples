...
function modelPM(xy_c::Array{Float64,2}, xy_f::Array{Float64,2}, p::Int)
    ...
    @constraint(model, r1[i in 1:n], sum(y[i,j] for j in 1:m) == 1)
    @constraint(model, r2, sum(x[j] for j in 1:m) == p )
    ...
    JuMP.set_optimizer_attribute(model, "tm_lim", 10 * 1000)
    JuMP.optimize!(model)

    xval = JuMP.value.(x) .> 1.0 - eps()
